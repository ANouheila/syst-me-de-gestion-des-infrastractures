    ...
    if(isset($_POST['modifier'])){
       ...
       /* $consistance=addslashes($consistance);
        ...
        $etapes=addslashes($etapes);*/
            ...
            $demande=$bdd->prepare("UPDATE projets SET situe_foncier=:foncier,coor_x=:coor_x,coor_y=:coor_y,superficie_glob=:glob,superficie_const=:superficie_const,consistance_physique=:consistance,etapes=:etapes,observation=:observation,type=:type WHERE id=:id");
            ...
            foreach($bureaux as $bureau=>$value){
                    $requet=$bdd->prepare("UPDATE $value SET nom=:nom,mail=:mail,tele=:tele,depense=:depense WHERE id_projet=:id");
             ...
             if($_FILES['plan']['size']<=$taillemax){
                 ...
                 //strchr nous permet de divider la chiane en deux parties
                   ...
                   if(in_array($extensionUpload,$extensionsvalides)){
                    ...
                    $connexion=$bdd->prepare("UPDATE projets SET plans='$avatar' WHERE id=$get_id");
                    $connexion->execute();
                }
                    else{
        ...
        if(!empty($_FILES['plan_masse']['name']))
        {
            $taillemax=2097152 ;
            //le nombre octet la taille maximale du photo
            $extensionsvalides=array('pdf','doc','jpg','jpeg','gif','png','docx','xlsx','pptx');
             if($_FILES['plan_masse']['size']<=$taillemax){
                 //substr ne permet d'ignorer'le premiere caractere de la chaine .jpg=>jpg(car on amis 1)
                 //strchr nous permet de divider la chiane en deux parties
                   $extensionUpload=strtolower(substr(strrchr($_FILES['plan_masse']['name'],'.'),1)) ;//pour extraire l'extension
                   if(in_array($extensionUpload,$extensionsvalides)){
                    //la variable chemin stocke le chemin dans lequelle sera upload notre photo
                    $chemin="../projets/plan_masse/".$get_id.".".$extensionUpload;
                    //tmp_name c'est le dossier ou existe le fichier auparavant
                    $resultat=move_uploaded_file($_FILES['plan_masse']['tmp_name'],$chemin);
                    if($resultat)
                    {
                         //on va ajouter la photo du profil dans la base de donnes
                         $avatar=$get_id.".".$extensionUpload;
                    $connexion=$bdd->prepare(" UPDATE projets SET plan_masse='$avatar' WHERE id=?");
                    ...
                    <div class="container-fluid">
                        <div class="ajout-formulaire">
                                    <h3><?=$projet['intitule_projet']?></h3>
                                      <form class="row g-3" method="post" action="modifier_etude.php?id=<?=$get_id?>" enctype="multipart/form-data">
                                    ...
                                    <div class="col-md-3">
                                        ...
                                        <input type="text" class="form-control" id="validationServer01" value="<?=$projet['type']?>" name="type" required>
                                    </div>

                                    <div class="col-md-3">
                                        <label for="validationServer01" class="form-label">Situation du foncier</label>
                                        <input type="text" class="form-control" id="validationServer01" value="<?=$projet['situe_foncier']?>" name="foncier" required>
                                    </div>
                                    <div class="col-md-3">
                                        ...
                                        <div class="input-group has-validation">
                                            ...
                                            <input type="number" class="form-control" id="validationServer02" name="superficie_glob" value="<?=$projet['superficie_glob']?>" required>
                                        </div>
                                        </div>
                                    <div class="col-md-3">
                                        ...
                                        <div class="input-group has-validation">
                                        <span class="input-group-text" id="inputGroupPrepend3">m<sup>2</sup></span>
                                        <input type="number" class="form-control" id="validationServerUsername" name="superficie_const" value="<?=$projet['superficie_const']?>"aria-describedby="inputGroupPrepend3 validationServerUsernameFeedback">
                                        </div>
                                    </div>
                                    ...
                                    foreach($bureaux as $bureau=>$value){
                                        ...
                                        $requete->execute(array($projet['id']));
                                    ...
                                    <div class="col-md-3">
                                        <input type="text" class="form-control" id="validationServer01" name="nom_<?=$value?>" value=<?=$requete['nom']?> placeholder="<?=$requete['nom']?>" required>
                                    </div>
                                    <div class="col-md-3">
                                        <div class="input-group has-validation">
                                        <span class="input-group-text" id="inputGroupPrepend3">Dhs</span>
                                        <input type="number" class="form-control" id="validationServerUsername" name="finan_<?=$value?>" value=<?=$requete['depense']?> placeholder="Dépense Financiere" aria-describedby="inputGroupPrepend3 validationServerUsernameFeedback" required>
                                        </div>
                                    </div>
                                    <div class="col-md-3">
                                        <input type="text" class="form-control" id="validationServer02" name="tele_<?=$value?>" value=<?=$requete['tele']?> placeholder="N° Télephone" required>
                                    </div>
                                    <div class="col-md-3">
                                        <input type="mail" class="form-control" id="validationServer02" name="mail_<?=$value?>" value=<?=$requete['mail']?> placeholder="adresse mail" required>
                                    </div>
                                       ...
                                       <div class="input-group">
                                        ...
                                        <div class="col-md-6">
                                            ...
                                            <label for="validationServer02" class="form-label">Plan de masse</label>
                                                <input type="file" class="form-control file" name="plan_masse" aria-label="file example">
                                            </div>
                                        </div>
                                        ...
                                        <div class="col-md-6">
                                                ...
                                                <label for="validationServer02" class="form-label">le site (Format image)</label>
                                                    <input type="file" class="form-control file" name="site" aria-label="file example" >
                                                </div>
                                        </div>
                                        ...
                                        <div class="col-md-6 obs">
                                                ...
                                                <label for="validationServer02" class="form-label">les coordonnées Lambert</label>
                                                    <input type="number" step="0.1" class="form-control" name="coor_x" aria-label="file example" placeholder="<?=$projet['coor_x']?>">
                                                </div>
                                        </div>
                                        <div class="col-md-6">
                                                <div>
                                                <br><br>
                                                    <input type="number" step="0.1" class="form-control" name="coor_y" aria-label="file example" placeholder="<?=$projet['coor_y']?>">
                                                </div>
                                        </div>
                                        ...
                                        <div class="col-md-12">
                                                <div>
                                                <label for="validationServer02" class="form-label">Les plans Du projet</label>
                                                    <input type="file" class="form-control file" name="plan" aria-label="file example">
                                                </div>
                                        </div>
                                        <div class="col-md-12">
                                        <div>
                            ...
                            <div class="col-md-12">
                                ...
                                <div class="col-md-10"></div>
                                    <div class="col-md-2">
                                        <a><button class="btn btn-primary convention" type="submit" name="modifier">Modifier</button></a>
                                    </div>
                                    </form>                        
                                </div>
                    </div>
